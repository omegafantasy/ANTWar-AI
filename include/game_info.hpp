 ...
 */
struct GameInfo {
    short round;                                   ///< Current round number
    ...
    short coins[2];                                ///< Coins of both sides: "coins[player_id]"
    ...
    short super_weapon_cd[2][SuperWeaponCount]; ///< Super weapon cooldown of both sides: "super_weapon_cd[player_id]"
    short old_count[2];
    short die_count[2];

    short next_ant_id;   ///< ID of the next generated ant.
    short next_tower_id; ///< ID of the next built tower.

    GameInfo() {}

    GameInfo(unsigned long long seed)
        : round(0), bases{Base(0), Base(1)}, coins{COIN_INIT, COIN_INIT}, super_weapon_cd{}, old_count{}, die_count{},
          next_ant_id(0), next_tower_id(0) {
     ...
     */
    template <typename T, typename Pred> optional<T> find_one(const std::vector<T> &v, Pred pred) const {
     ...
     */
    template <typename T, typename Pred> std::vector<T> find_all(const std::vector<T> &v, Pred pred) const {
     ...
     */
    std::vector<Ant> all_ants() const { return ants; }
    ...
    int ant_of_id_by_index(int id) const {
        auto it = std::find_if(ants.begin(), ants.end(), [&](const Ant &a) { return a.id == id; });
        if (it != ants.end())
     ...
     */
    std::vector<Tower> all_towers() const { return towers; }
    ...
    optional<Tower> tower_at(int x, int y) const {
        return find_one(towers, [x, y](const Tower &t) { return t.x == x && t.y == y; });
    }
    ...
    optional<Tower> tower_of_id(int id) const {
        return find_one(towers, [id](const Tower &t) { return t.id == id; });
    }
    ...
    void upgrade_tower(int id, TowerType type) {
        auto it = std::find_if(towers.begin(), towers.end(), [&](const Tower &t) { return t.id == id; });
        if (it != towers.end()) {
            it->upgrade(type);
        }
    }
    ...
    void upgrade_generation_speed(int player_id) { bases[player_id].upgrade_generation_speed(); }

    void upgrade_generated_ant(int player_id) { bases[player_id].upgrade_generated_ant(); }

    /**
     * @brief Set the number of coins for a player.
     */
    void set_coin(int player_id, int value) { coins[player_id] = value; }

    /**
     * @brief Update the number of coins for a player.
     */
    void update_coin(int player_id, int change) { coins[player_id] += change; }

    /**
     * @brief Set the hp of base for a player.
     */
    void set_base_hp(int player_id, int value) { bases[player_id].hp = value; }

    /**
     * @brief Update the hp of base for a player.
     */
    void update_base_hp(int player_id, int change) { bases[player_id].hp += change; }
            ...
            for (int j = 0; j < MAP_SIZE; ++j)
                for (int k = 0; k < MAP_SIZE; ++k)
                    if (MAP_PROPERTY[j][k] >= 0)
                        pheromone[i][j][k] = PHEROMONE_ATTENUATING_RATIO * pheromone[i][j][k] +
    ...
    int tower_num_of_player(int player_id) const {
        return std::count_if(towers.begin(), towers.end(),
                             [player_id](const Tower &tower) { return tower.player == player_id; });
    }
    ...
    bool is_operation_valid(int player_id, const Operation &op) const {
        switch (op.type) {
        case BuildTower:
            return is_valid_pos(op.arg0, op.arg1) && is_highland(player_id, op.arg0, op.arg1) &&
                   !is_shielded_by_emp(player_id, op.arg0, op.arg1);
        case UpgradeTower: {
            auto t = tower_of_id(op.arg0);
            return t && t.value().player == player_id && t.value().is_upgrade_type_valid(op.arg1) &&
                   !is_shielded_by_emp(t.value());
        }
        case DowngradeTower: {
            auto t = tower_of_id(op.arg0);
            return t && t.value().player == player_id && !is_shielded_by_emp(t.value());
        }
        case UseLightningStorm:
        case UseEmpBlaster:
        case UseDeflector:
        case UseEmergencyEvasion:
            return is_valid_pos(op.arg0, op.arg1) && super_weapon_cd[player_id][op.type % 10] <= 0;
    ...
    bool is_operation_valid(int player_id, const std::vector<Operation> &ops, const Operation &new_op) const {
        ...
        case OperationType::UseEmergencyEvasion:
            collide = std::any_of(ops.begin(), ops.end(), [&](const Operation &op) { return op.type == new_op.type; });
            break;
    ...
    int next_move(const Ant &ant) const {
        ...
        // Data
        int target_x = Base::POSITION[!ant.player][0], target_y = Base::POSITION[!ant.player][1];
        ...
        for (int i = 0; i < 6; ++i) {
            ...
            int x = ant.x + OFFSET[ant.y % 2][i][0], y = ant.y + OFFSET[ant.y % 2][i][1];
        ...
        // Get max
        auto p = std::max_element(phero, std::end(phero), [phero](const double ph1[], const double ph2[]) {
     ...
     */
    static int downgrade_tower_income(int type) { return upgrade_tower_cost(type) * TOWER_DOWNGRADE_REFUND_RATIO; }
     ...
     */
    static int use_super_weapon_cost(int type) { return SUPER_WEAPON_INFO[type][3]; }
     ...
     */
    bool is_shielded_by_emp(const Tower &tower) const { return is_shielded_by_emp(tower.player, tower.x, tower.y); }
    ...
    void show() const {
        ...
        for (auto &tower : towers)
            fout << tower.id << '\t' << tower.player << "\t\t" << tower.x << '\t' << tower.y << '\t' << tower.type
                 << '\t' << tower.cd << std::endl;
        // Ants
        fout << "Ants:" << std::endl;
        fout << "id\tplayer\tx\ty\thp\tage\tstate" << std::endl;
        for (auto &ant : ants)
            fout << ant.id << '\t' << ant.player << "\t\t" << ant.x << '\t' << ant.y << '\t' << ant.hp << '\t'
                 << ant.age << '\t' << ant.state << std::endl;
        // Coins
    ...
    void dump(std::ofstream &fout) const {
        ...
        for (auto &ant : ants)
            fout << ant.id << ' ' << ant.player << ' ' << ant.x << ' ' << ant.y << ' ' << ant.hp << ' ' << ant.level
                 << ' ' << ant.age << ' ' << ant.state << std::endl;
        // Coins
    ...
    void cerrdump() const {
        ...
        fprintf(stderr, "%d\n", towers.size());
        for (auto &tower : towers)
            fprintf(stderr, "%d %d %d %d %d %d\n", tower.id, tower.player, tower.x, tower.y, tower.type, tower.cd);
        // Ants
        fprintf(stderr, "%d\n", ants.size());
        for (auto &ant : ants)
            fprintf(stderr, "%d %d %d %d %d %d %d %d\n", ant.id, ant.player, ant.x, ant.y, ant.hp, ant.level, ant.age,
                    ant.state);
        // Coins
        fprintf(stderr, "%d %d\n", coins[0], coins[1]);
        // Hp
        fprintf(stderr, "%d %d\n", bases[0].hp, bases[1].hp);
        // Pheromone
        for (int player = 0; player < 2; ++player) {
            for (int i = 0; i < MAP_SIZE; ++i) {
                for (int j = 0; j < MAP_SIZE; ++j) {
                    fprintf(stderr, "%.4f ", pheromone[player][i][j]);
                }
                fprintf(stderr, "\n");
            }
        }
