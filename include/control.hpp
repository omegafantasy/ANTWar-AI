...
class Controller {
  private:
    ...
    void update_ant(const Ant &a) {
        auto it = std::find_if(info.ants.begin(), info.ants.end(), [a](const Ant &x) { return x.id == a.id; });
     ...
     */
    Controller(InitInfo init_info) : info(init_info.second), self_player_id(init_info.first) {}
  ...
  public:
     ...
     */
    const GameInfo &get_info() const { return info; }

    /**
     * @brief Get added operations.
     * @return A read-only (constant) reference to self operations.
     */
    const std::vector<Operation> &get_self_operations() const { return self_operations; }

    /**
     * @brief Get saved opponent's operations.
     * @return A read-only (constant) reference to opponent operations.
     */
    const std::vector<Operation> &get_opponent_operations() const { return opponent_operations; }
     ...
     */
    void read_opponent_operations() { opponent_operations = ::read_opponent_operations(); }
     ...
     *        and arguments. This function will check validness of the operation and add it to "self_operations" if
     * valid.
     ...
     */
    void send_self_operations() const { send_operations(self_operations); }
};